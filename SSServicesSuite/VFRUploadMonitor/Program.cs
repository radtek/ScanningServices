...
namespace VFRUploadMonitor
{
    static class Program
    {
        ...
        public static void AnalizeJob(string logJobName, JobExtended job)
        {
            try
            {
                lock (lockObj)
                {
                    ...
                    // Get Batches in "Waiting for VFRUploadMonitor"
                    nlogger.Info("      Getting Batches in Waiting for VFRUploadMonitor ...");
                    Console.Out.WriteLineAsync("      Getting Batches in Waiting for VFRUploadMonitor ...");
                    ResultBatches resulBatches = new ResultBatches();
                    resulBatches = DBTransactions.GetBatches(logJobName, job.JobName.Trim(),"Waiting for VFRUploadMonitor");
        ...
        public static void VFRUploadMonitorProcess(string logJobName, int jobID)
        {
            try
            {
                lock (lockObj)
                {
                    ...
                    if (jobID == 0)
                    {
                        ...
                        localCronJobsList = VFRUploadMonitorService.cronJobsList;
        ...
        static void Main(string[] args)
        {
...
#if DEBUG
            VFRUploadMonitorService service1 = new VFRUploadMonitorService(args);
            ...
            ServiceBase[] ServicesToRun;
                        ...
                        {
                            new  VFRUploadMonitorService(args)
                        };
        ...
        {            
            ...
            {                
                ...
                resultProcess = DBTransactions.GetProcessByName("VFR Upload Monitor");
                if (resultProcess.ReturnCode == 0)
                {
                    ...
                    foreach (ScanningServicesDataObjects.GlobalVars.Process process in resultProcess.ReturnValue)
                    {
                        ...
                        if (VFRUploadMonitorService.hostName == process.StationName.Trim())
                        {
                            ...
                            foreach (ScanningServicesDataObjects.GlobalVars.Process cronJob in VFRUploadMonitorService.cronJobsList)
                            {
                            ...
                            if (!cronJobFound && process.EnableFlag)
                            {
                                ...
                                if (process.EnableFlag)
                                {
                                    ...
                                    if (Quartz.CronExpression.IsValidExpression(process.ScheduleCronFormat))
                                    {
                                        ...
                                        VFRUploadMonitorService.cronJobsList.Add(jobProcess);
                                        ...
                                        foreach (ScanningServicesDataObjects.GlobalVars.Process cronJob in VFRUploadMonitorService.cronJobsList)
                                        {
                                            if (cronJob.JobID == process.JobID && cronJob.ProcessID == process.ProcessID)
                                            {
                                                lock (lockObj)
                                                {
                                                    ...
                                                    Console.ForegroundColor = ConsoleColor.Magenta;
                                                    VFRUploadMonitorService.cronJobsList.Remove(cronJob);
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                {                    
                    ...
                    VFRUploadMonitor.Program.VFRUploadMonitorProcess(logJobName, Convert.ToInt32(context.JobDetail.Key.Name.Replace("JOB-", "")));
