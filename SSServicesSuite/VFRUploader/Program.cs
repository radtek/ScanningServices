...
namespace VFRUploader
{
    static class Program
    {
        ...
        public static void AnalizeJob(string logJobName, JobExtended job)
        {
            try
            {
                lock (lockObj)
                {
                    ...
                    // Get Batches in "Waiting for VFRUploader"
                    nlogger.Info("      Getting Batches in Waiting for VFRUploader ...");
                    Console.Out.WriteLineAsync("      Getting Batches in Waiting for VFRUploader ...");
                    ResultBatches resulBatches = new ResultBatches();
                    resulBatches = DBTransactions.GetBatches(logJobName, job.JobName.Trim(),"Waiting for VFRUploader");
        ...
        public static void VFRUploaderProcess(string logJobName, int jobID)
        {
            try
            {
                lock (lockObj)
                {
                    ...
                    if (jobID == 0)
                    {
                        ...
                        localCronJobsList = VFRUploaderService.cronJobsList;
...
#if DEBUG
            VFRUploaderService service1 = new VFRUploaderService(args);
        ...
        {            
            ...
            {                
                ...
                resultProcess = DBTransactions.GetProcessByName("VFR Uploader");
                if (resultProcess.ReturnCode == 0)
                {
                    ...
                    foreach (ScanningServicesDataObjects.GlobalVars.Process process in resultProcess.ReturnValue)
                    {
                        ...
                        if (VFRUploaderService.hostName == process.StationName.Trim())
                        {
                            ...
                            foreach (ScanningServicesDataObjects.GlobalVars.Process cronJob in VFRUploaderService.cronJobsList)
                            {
                            ...
                            if (!cronJobFound && process.EnableFlag)
                            {
                                ...
                                if (process.EnableFlag)
                                {
                                    ...
                                    if (Quartz.CronExpression.IsValidExpression(process.ScheduleCronFormat))
                                    {
                                        ...
                                        VFRUploaderService.cronJobsList.Add(jobProcess);
                                        ...
                                        foreach (ScanningServicesDataObjects.GlobalVars.Process cronJob in VFRUploaderService.cronJobsList)
                                        {
                                            if (cronJob.JobID == process.JobID && cronJob.ProcessID == process.ProcessID)
                                            {
                                                lock (lockObj)
                                                {
                                                    ...
                                                    Console.ForegroundColor = ConsoleColor.Magenta;
                                                    VFRUploaderService.cronJobsList.Remove(cronJob);
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                {                    
                    ...
                    VFRUploader.Program.VFRUploaderProcess(logJobName, Convert.ToInt32(context.JobDetail.Key.Name.Replace("JOB-", "")));
