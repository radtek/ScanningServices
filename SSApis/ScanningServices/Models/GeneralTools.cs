...
namespace ScanningServices.Models
{
    public class GeneralTools
    {
        ...
        // The CSV Class implementation was taken from this site: //http://www.blackbeltcoder.com/Articles/files/reading-and-writing-csv-files-in-c
        /// <summary>
            ...
            if (!string.IsNullOrEmpty(scheduleJS))
            {
                ...
                if (schedule.repeatTaskFlag)
                {
                    if (schedule.repeatEveryHoursFlag)
                    {
                        ...
                        {
                            //hours = schedule.taskBeginHour.ToString() + "-" + schedule.taskEndHour.ToString() + "/" + schedule.repeatTaskTimes.Length.ToString();
                            // Take a look at https://crontab.guru/#0/2_10-20/3_*_*_*
                            // We want to have Repeat task every Radio Button + entry for Minutes, and entry for Hours, and Entry for Range
                            // So add to the databse int Repeat-Every-Hour, and int Repeat-Every-Minute
